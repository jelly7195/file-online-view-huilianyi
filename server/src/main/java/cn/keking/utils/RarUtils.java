...
//    private static final String fileDir = ConfigConstants.getFileDir();
  ...
  //    Pattern p = Pattern.compile("\\s|\\+|#|&|=|\\p{P}");
    ...
    public static List<ZtreeNodeVo> getTree(String rootPath) {
        ...
        File file = new File(ConfigConstants.getFileDir()+rootPath);
    ...
    private static ZtreeNodeVo traverse(File file) {
        ...
        pathNodeVo.setId(file.getAbsolutePath().replace(ConfigConstants.getFileDir(), "").replace("\\", "/"));
        pathNodeVo.setName(file.getName());
        pathNodeVo.setPid(file.getParent().replace(ConfigConstants.getFileDir(), "").replace("\\", "/"));
        if (file.isDirectory()) {
