...
//    private static final String fileDir = ConfigConstants.getFileDir();
    ...
    public static ReturnResponse<String> downLoad(FileAttribute fileAttribute, String fileName) {
        ...
        if (fileAttribute.isCompressFile()) { //压缩包文件 直接赋予路径 不予下载
            String compressFilePath = fileAttribute.getCompressFilePath();
            // URL解码 compressFilePath
            if (compressFilePath != null) {
                try {
                    compressFilePath = java.net.URLDecoder.decode(compressFilePath, "UTF-8");
                } catch (java.io.UnsupportedEncodingException e) {
                    logger.error("解码 compressFilePath 失败", e);
                }
            }
            // 从 compressFilePath 中提取压缩包内的相对路径（去掉压缩包key前缀）
            String compressFileKey = fileAttribute.getCompressFileKey();
            if (compressFilePath != null && compressFilePath.startsWith(compressFileKey + "/")) {
                compressFilePath = compressFilePath.substring(compressFileKey.length() + 1);
            }
            String fullPath = fileAttribute.getCompressFileKey() + "/" + compressFilePath;
            response.setContent(ConfigConstants.getFileDir() + fullPath);
            response.setMsg(fileName);
            return response;
        }
        // 如果文件是否已经存在、且不强制更新，则直接返回文件路径
        if (KkFileUtils.isExist(realPath) && !fileAttribute.forceUpdatedCache()) {
            response.setSize(new File(realPath).length());
        ...
        try {
            ...
            logger.error("文件下载失败！realPath：{}", realPath);
            logger.error("文件下载失败！异常：{}", ExceptionUtils.getStackTrace(e));
            response.setCode(1);
            response.setContent(null);
